%-------------------------------------------------------------------------%
%---------------------- 3rd September 2023 -------------------------------%
%------------  Design of Multi UAV Superformula Guidance -----------------%
%-------------------------------------------------------------------------%
...
global Vg rd k1 wd xc yc alpha1 alpha2 alpha3 r1 r2 r3 te r4 alpha4 m n1 n2 n3 a b del
Vg = 10;
rd = 50;
k1 = 0.5;
wd = 0.25;
...
[xd,yd] = fun_superformula(psi);

tspan = [0 60];
x_initial = [200;200;180;-180;170;-170; 190;-195];

options = odeset('RelTol',1e-8,'AbsTol',1e-8);

%%%%%%%%%%%%%%%%%  ode solver   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[t,x] = ode45(@(t,x)fun_ode_sf(t,x) ,tspan, x_initial,options);
...
legend(ax1,[h14 h15 h16 h18], 'UAV 1','UAV 2','UAV 3','UAV 4','Fontsize',leg_fnt,'NumColumns',4);
...
% axis(ax1,'equal')
myWriter = VideoWriter('Multi_UAV Rectangle','MPEG-4');
open(myWriter);
for i = 1:10:length(x)
%     if i==1
...
function out = fun_ode_sf(t,x)
global k1  rd wd xc yc alpha1 alpha2 alpha3 r1 r2 r3 te alpha4 r4 m n1 n2 n3 a b del
if t ==0
    r1 = [];
    r2 = [];
    r3 = [];
...
fa_1 = abs(cos(m*((theta1-del)./4))./a).^n2 ;
% df1dgamma = -((m*n2)/(4*a^2))*(abs(cos(m*(theta1./4))./a).^(n2 -2)).*(sin(m*(theta1./4))).*(cos(m*(theta1./4))) ;
fb_1 = abs(sin(m*((theta1-del)./4))./b).^n3 ;
% df2dgamma = ((m*n3)/(4*b^2))*(abs(sin(m*(theta1./4))./b).^(n3 -2)).*(sin(m*(theta1./4))).*(cos(m*(theta1./4))) ;
Rd_1 = (fa_1 + fb_1).^(-1/n1);

fa_2 = abs(cos(m*((theta2-del)./4))./a).^n2 ;
% df1dgamma = -((m*n2)/(4*a^2))*(abs(cos(m*(theta1./4))./a).^(n2 -2)).*(sin(m*(theta1./4))).*(cos(m*(theta1./4))) ;
fb_2 = abs(sin(m*((theta2-del)./4))./b).^n3 ;
% df2dgamma = ((m*n3)/(4*b^2))*(abs(sin(m*(theta1./4))./b).^(n3 -2)).*(sin(m*(theta1./4))).*(cos(m*(theta1./4))) ;
Rd_2 = (fa_2 + fb_2).^(-1/n1);

fa_3 = abs(cos(m*((theta3-del)./4))./a).^n2 ;
% df1dgamma = -((m*n2)/(4*a^2))*(abs(cos(m*(theta1./4))./a).^(n2 -2)).*(sin(m*(theta1./4))).*(cos(m*(theta1./4))) ;
fb_3 = abs(sin(m*((theta3-del)./4))./b).^n3 ;
% df2dgamma = ((m*n3)/(4*b^2))*(abs(sin(m*(theta1./4))./b).^(n3 -2)).*(sin(m*(theta1./4))).*(cos(m*(theta1./4))) ;
Rd_3 = (fa_3 + fb_3).^(-1/n1);

fa_4 = abs(cos(m*((theta4-del)./4))./a).^n2 ;
% df1dgamma = -((m*n2)/(4*a^2))*(abs(cos(m*(theta1./4))./a).^(n2 -2)).*(sin(m*(theta1./4))).*(cos(m*(theta1./4))) ;
fb_4 = abs(sin(m*((theta4-del)./4))./b).^n3 ;
% df2dgamma = ((m*n3)/(4*b^2))*(abs(sin(m*(theta1./4))./b).^(n3 -2)).*(sin(m*(theta1./4))).*(cos(m*(theta1./4))) ;
Rd_4 = (fa_4 + fb_4).^(-1/n1);
...
out(1,1) = -k11*e1*cos(theta1) - Rd_1.*(wd + h1)*(-sin(theta1));
out(2,1) = -k11*e1*sin(theta1) - Rd_1.*(wd + h1)*(cos(theta1));

out(3,1) = -k21*e2*cos(theta2) - Rd_2.*(wd + h2)*(-sin(theta2));
out(4,1) = -k21*e2*sin(theta2) - Rd_2.*(wd + h2)*(cos(theta2));

out(5,1) = -k31*e3*cos(theta3) - Rd_3.*(wd + h3)*(-sin(theta3));
out(6,1) = -k31*e3*sin(theta3) - Rd_3.*(wd + h3)*(cos(theta3));

out(7,1) = -k41*e4*cos(theta4) - Rd_4.*(wd + h4)*(-sin(theta4));
out(8,1) = -k41*e4*sin(theta4) - Rd_4.*(wd + h4)*(cos(theta4));
% 
% kz1 = 5;h = 0.02;
% out(9,1) = kz1*(h - x(9));
% out(10,1) = kz1*(h - x(10));
...
function [xd,yd] = fun_superformula(gamma)
global V m n1 n2 n3 k1 a b xc yc del

f1 = abs(cos(m*((gamma-del)./4))./a).^n2 ;
f2 = abs(sin(m*((gamma-del)./4))./b).^n3 ;
