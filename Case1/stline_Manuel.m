...
%Parameters
v=1;%m/s
Lmax = 3 ; Lmin = 0.1;lambda = 1;

L0_arr = [0.1 3];

for i = 1:length(L0_arr)
    % for i = 1:2
    L0 = L0_arr(i);
    % else
    %  L0 = L0_var;   
...
% figure(1)
% subplot(4,1,1)
% plot(tout, yref*ones(size(tout)),'k')
% hold all
% subplot(4,1,2)
...
%Minimum distance and distance vector
 ...
 L0_var = (Lmax - Lmin)*exp(-lambda*d) + Lmin;
L0var(k) = L0_var ;
...
% %Plotting Trajectories for each L0
%  figure(1)
% subplot(4,1,1); %Position
% plot(tout, X(2,:))
% hold all
% 
% subplot(4,1,2); %Curvature
% plot(tout, c_v)
% hold all
% 
% subplot(4,1,3); %Distance to the path
% plot(tout, d_v)
% hold all
% 
% subplot(4,1,4)%Heading error eta
% plot(tout, eta_v)
% hold all
...
h12 = plot(X(1,:),X(2,:),'--','color',color(i,:),'LineWidth',3,'DisplayName',['$$ L_0 = \ $$' num2str(L0_arr(i)), '\ m']) ;hold on;grid on;
else
 h12 = plot(X(1,:),X(2,:),'--','color',color(i,:),'LineWidth',3,'DisplayName',['$$ L_0 = \ $$' num2str(L0_arr(i)), '\ m']) ;hold on;grid on;   
end
ax1 = gca;
...
figure(2)
h2 = plot(tout,d_v,'--','color',color(i,:),'LineWidth',3,'DisplayName',['$$ L_0 = \ $$' num2str(L0_arr(i)), '\ m']) ;hold on;grid on;
ax2 = gca;
ax2.FontSize = ax_fnt;
box on                        % Switch on the box around the axis
...
ylabel(ax2,'Cross-track error $$d$$, m','Fontsize',lbl_fnt);

figure(3)
h32 = plot(tout,wrapToPi(eta_v)*(180/pi),'--','color',color(i,:),'linewidth',3,'DisplayName',['$$ L_0 = \ $$' num2str(L0_arr(i)), '\ m']) ;hold on;grid on;
ax3 = gca;
ax3.FontSize = ax_fnt;
box on                        % Switch on the box around the axis
...
figure(4)
h4 = plot(tout,c_v,'--','color',color(i,:),'LineWidth',3,'DisplayName',['$$ L_0 = \ $$' num2str(L0_arr(i)), '\ m']) ;hold on;grid on;
ax4 = gca;
ax4.FontSize = ax_fnt;
box on                        % Switch on the box around the axis
