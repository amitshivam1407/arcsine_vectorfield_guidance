...
x_initial = [x0;y0;kaid0;kaidot0;0.1];
...
%%%%%%%%%%%%%%%%%  ode solver   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[t,x] = ode45(@(t,x)fun_stline(t,x) ,tspan, x_initial,options);
...
k1 = 50; k22 = 10; ki = 5;
    ...
    %  spsi = k_kai*epsi + k_kaidot*epsidot + ki*x(6);
     ...
     % spsi = k_kai*x(3) + k_kaidot*x(4) + ki*x(6);

     % spsi = k_kai*x(3) + k_kaidot*x(4) ;

     spsi = k_kai*(kaid - x(3)) + k_kaidot*(kaid_dot - x(4)) ;
    ...
    % u = -(k_kai/k_kaidot)*x(4)-(ki/k_kaidot)*x(3) -(k1/k_kaidot)*sqrt(abs(spsi))*spsi_sat-(k22/k_kaidot)*I_u;

     % u = -(k_kai/k_kaidot)*x(4) -(k1/k_kaidot)*sqrt(abs(spsi))*spsi_sat-(k22/k_kaidot)*I_u;

     u = (k_kai/k_kaidot)*(kaid_dot - x(4)) +(k1/k_kaidot)*sqrt(abs(spsi))*spsi_sat+(k22/k_kaidot)*I_u;
