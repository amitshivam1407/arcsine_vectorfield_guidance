function figure_plot = fun_stlineplot(t,X,Y,xdot,ydot,xdot_mod,ydot_mod,x1,x2,...
    x_ini,y_ini,x_end,y_end,kaidot_d,kaidot_a,kappa_d,kappa_a,...
    kai_d, x3)
...
hn = quiver(ax1,X,Y,xdot_mod,ydot_mod,'color',[0  0.25   0.5],'linewidth',1);hold(ax1,'on');
...
% ax1.FontSize = 13;
l = legend(ax1,[h hn xl h1] ,'Vector field','Modified VF','Desired path','UAV trajectory');
...
f3;
h3 = plot(ax3,t,kaidot_d);hold(ax3,'on');grid(ax3,'on');
h3.Color = red;                % Color of plot
h3.LineStyle = '-';             % Line style ('--' -> Dash, '-.' -> Dot+Dash, ':' -> Dot)
h3.LineWidth = lw_;             % Plot line width
h3.Marker = 'none';             % Marker type ('s' -> Square, 'o' -> Circle, 'd' -> Diamond)
...
h3.MarkerEdgeColor = 'auto';    % Marker edge color
h3.MarkerFaceColor = 'none';    % Marker face color
h31 = plot(ax3,t,kaidot_a);hold(ax3,'on');grid(ax3,'on');
...
f4;
% figure_pos = figure('position', [p_x0, p_y0, p_width, p_height]); 
% h4 = plot(ax4,x1,kappa_d);hold(ax4,'on') ;grid(ax4,'on')
...
f6;
% h6 = plot(ax6,x1,kai_d*(180/pi));hold(ax6,'on') ;grid(ax6,'on')
