...
k_prop = 0.005;
% k_prop = [2*10^(-5) 2.4*10^(-4) 4.6*10^(-4) 6.8*10^(-4) 9*10^(-4) 0.0011 0.0013 0.0016 0.0018 0.0020 0.003 0.004 0.005 0.006 0.007 0.008 0.009 0.01];
% k_prop = 2*10^(-5):(0.01-2*10^(-5))/19 :0.01;
...
[val_max_prop(i) ,ind_prop(i)] = max(kappa_actual_prop);
...
[val_max_Nelson(i), ind_Nelson(i)] = max(kappa_actual_Nelson);
...
% ylabel(ax5,'Course \ angle, \ $ \gamma$, \ $ (^\circ) $','Fontsize',18)
...
% plot(ax7,x_max_prop,val_max_prop,'-s','MarkerSize',10,...
...
ylabel(ax11,' Control effort \ ratio','Fontsize',18');
...
ax12.FontSize = 13;
xlabel(ax12,' Proposed \ gain, \ $k_{s}$ ','Fontsize',18);
ylabel(ax12,' Maximum \ curvature \ ratio','Fontsize',18');
...
ax13.FontSize = 13;
xlabel(ax13,' Proposed \ gain, \ $k_{s}$ ','Fontsize',18);
ylabel(ax13,' Settling \ time \ ratio','Fontsize',18');
...
ax14.FontSize = 13;
xlabel(ax14,' Proposed \ gain, \ $k_{s}$ ','Fontsize',18);
ylabel(ax14,' $ \% $ Reduction \ in \ settling \ time ','Fontsize',18');
...
% function out = fun_comp_circular_nowind(t,x)
% global k_kai k_kaidot rd k2 Vgd k1 flag1 t_flag1 flag2 t_flag2
% 
...
% out(6,1) = k_kai*(kaid - x(5)) + k_kaidot*(kaid_dot - x(6))  ;
% 
% kaid_Nelson = x(8) + pi/2 + atan(k1.*(x(7) - rd)) ;
% gamma_dot_Nelson = (Vgd./x(7) ).*sin(kaid_Nelson - x(8)) ;
% kaid_dot_Nelson = gamma_dot_Nelson - (k1^2*Vgd.*(x(7) -rd))./((1+k1^2*(x(7) -rd).^2).^(3/2));
% 
