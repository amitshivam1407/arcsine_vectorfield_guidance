...
global A_amp omega  m k1 Va k_s tau  W psi_w A_w A_phiw epsilon nabla zeta_0 zeta_1 zeta_2 c0 c1 c2 c3 ...
...
for i = 1:length(X)
    for j = 1:length(X)
%         if e(i,j)< 0
          kaid(i,j) =  sech(k1*e(i,j))*(kai_p_des(1,j)) + tanh(k_s*e(i,j))*kai_o(i,j) ;
...
[t,x] = ode45(@(t,x)fun_sinusoidal(t,x,u,v,Time) ,tspan, x_initial,options);
...
omega11 = omega*exp(-x(:,1)./tau);
e_path = m*x(:,1) + A_amp*sin(omega11.*x(:,1)) - x(:,2);

for i = 1:length(t)
    if abs(e_path(i,1))  <= 0.1
        c(i) = abs(e_path(i,1)) ;
        c_arr = [c_arr abs(e_path(i,1))];
    else
        c_tran =[c_tran abs(e_path(i,1))];
%         c_arr_col(j,:) = c_arr ;
    end
...
figure(2)
plot(t,e_path,'m','LineWidth',3);hold on;grid on;
ax2 = gca;
ax2.FontSize = ax_fnt;
% Outer box setup
box on                        % Switch on the box around the axis
...
% plot(tez,sz,'linewidth',3,'DisplayName','Sliding variable');grid on;hold on;
% ax6 = gca;
% ax6.FontSize = ax_fnt;
% % Outer box setup
% box on                        % Switch on the box around the axis
...
function out = fun_sinusoidal(t,x,u,v,Time)
global Va A_amp omega tau m k1 W psi_w A_w A_phiw c0 c2 c1 epsilon tez uz zeta_1 zeta_2 sz k_s Va wxs wys nabla kai_ddot
...
omega1 = omega*exp(-x(1)/tau);


dydx =  m - (A_amp/tau)*omega1*cos(omega1.*x(1));
kai_p =  atan(dydx);
e =  m*x(1) + A_amp*sin(omega1.*x(1)) - x(2);

kai_o = pi/2 - asin(1./(1 + k_s*(e).^2)) ;

if e < 0
    kaid = sech(k1*e).*(kai_p) + tanh(k_s*e).*kai_o;
    kai = x(3) + kaid ;
    Vg = Wx.*cos(kai) + Wy.*sin(kai) + sqrt(Va^2 - (Wx.*sin(kai) - Wy.*cos(kai))) ;
    out(1,1) = Vg*cos(kai);
    out(2,1) = Vg*sin(kai);
    domega1dt =  m-(omega/tau)*exp(-x(1)/tau)*out(1,1) ;
    kaip_dot = -(A_amp*omega1.^2 *sin(omega1.*x(1))*(omega1.*out(1,1) + x(1).*domega1dt))./(1 + (tan(kai_p)).^2);
    factor_1 = (2*k_s)/((1 + k_s*(e).^2).*(sqrt(2*k_s + (k_s*e).^2)));
    epsilon_dot = A_amp*omega1.*cos(omega1.*x(1))*(omega1.*out(1,1) + x(1).*domega1dt) -out(2,1);
    ...
    out(1,1) = Vg*cos(kai);
    out(2,1) = Vg*sin(kai);
    domega1dt =  -(omega/tau)*exp(-x(1)/tau)*out(1,1) ;
    kaip_dot = -(A_amp*omega1.^2 *sin(omega1.*x(1))*(omega1.*out(1,1) + x(1).*domega1dt))./(1 + (tan(kai_p)).^2);
    factor_1 = (2*k_s)/((1 + k_s*(e).^2).*(sqrt(2*k_s + (k_s*e).^2)));
    epsilon_dot = A_amp*omega1.*cos(omega1.*x(1))*(omega1.*out(1,1) + x(1).*domega1dt) -out(2,1);
    kaio_dot = factor_1.*epsilon_dot;
...
% f_kaidot_val = get_num_kaid_ddot(A_amp,omega, x(1), t)
...
function f_kaidot_val = get_num_kaid_ddot(A_amp,omega, x1, t1)
% global A_amp omega
...
% fy = diff(f(t),y(t)) ;
% f_kai = atan(-fx/fy);
f_kaidot = diff(f_kai,t);
f_kaiddot = diff(f_kaidot,t);
f_kaidot_val = subs(f_kaidot,[A w x(t) t],[A_amp omega x1 t1])
end