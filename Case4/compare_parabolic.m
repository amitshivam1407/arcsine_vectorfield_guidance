...
global  Vgd k_kai k_kaidot k2 flag1 t_flag1 flag2 t_flag2 k_aseem a h k
a = .01;
h = 0;
k = -100;
...
[kaid0,kaid_dot0] = get_initialvalue(x0,y0,k2);


kai0_deg = rad2deg(wrapToPi(kaid0));
epsilon0 = -((y0 - k) - a*(x0 - h)^2);
...
legend(ax1,'Parabolic path','Proposed','Griffiths et al. 2006','Fontsize',leg_fnt)
axis(ax1,'equal')
...
global k_kai k_kaidot  Vgd flag2 t_flag2 k_aseem a h k
...
dydx = 2*a.*(y(1) - h);
epsilon = -((y(2) - k) - a.*(y(1) - h)^2);
kaio1 = atan(k_aseem.*(epsilon )) ;
kait1 = atan(dydx);
kaid_aseem = kait1  +  kaio1  ;
xdot1 = out(1,1);
ydot1 = out(2,1);
kait_dot1 = (2*a*xdot1)./(1 + (tan(kait1))^2);
epsilon_dot = -(ydot1 - 2*a*(y(1)-h)*xdot1);
...
global  a h  k k_kai k_kaidot Vgd flag1 t_flag1 flag2 t_flag2 k_aseem
...
function [kaid0,kaidot0] = get_initialvalue(x0,y0,k2)
global Vgd a h k
...
if epsilon0 < 0
    ...
    epsilon_dot0 = -(ydot0 - (2*a*(x0-h)*xdot0));
...
kaip_dot0 = (2*a*xdot0)./(1 + (tan(kait0))^2);
