...
%----------------- ode solver   ------------------------------------------%
[t,x] = ode45(@(t,x)fun_parabola_withwind(t,x) ,tspan, x_initial,options);
%
% %----------------------  parameters to plot   ----------------------------%
...
[kai_p, kai_o, kai_des,kaip_dot, kaio_dot, kaidot_des] = fun_propkaid(x(:,1),x(:,2),x(:,3),x(:,5));
...
legend(ax3,'$$\chi_{\mathrm{d}}$$','$$\chi$$','$$\chi_{\mathrm{p}}$$','$$\chi_{\mathrm{o}}$$','Fontsize',leg_fnt,'NumColumns',2);
...
plot(t,kaip_dot,'-','Color',maroon,'LineWidth',lw);hold on; grid on;
plot(t,kaio_dot,'-','Color','k','LineWidth',lw);hold on; grid on;
ax5 = gca;
ax5.FontSize = ax_fnt;
box on                      % Switch on the box around the axis
...
legend(ax5,'$$\dot{\chi}_{\mathrm{d}}$$','$$\dot{\chi}$$','$$\dot{\chi}_{\mathrm{p}}$$','$$\dot{\chi}_{\mathrm{o}}$$','Fontsize',leg_fnt,'NumColumns',2);
...
function [kai_p,kai_o,kai_des,kaip_dot, kaio_dot, kaidot_des] = fun_propkaid(x1,x2,x3,x5)
