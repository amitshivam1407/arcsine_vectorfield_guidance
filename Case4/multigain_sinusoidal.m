...
[t,x] = ode45(@(t,x)fun_sinusoidal(t,x,k2) ,tspan, x_init(:,i),options);
...
% %% ode solver equation  %%
% [t,x, te, ze, ie] = ode45(@(t,x) fun_sinusoidal(t,x,k2), tspan, x_init(:,i), option_x);
...
h13 = plot(x(:,1),x(:,2),'Color',color(i,:),'linewidth',lw,'DisplayName',['$$k_{\mathrm{g}} = \ $$', num2str(k(i))]);hold on;
...
h13 = plot(x(:,1),x(:,2),'Color',color(i,:),'linewidth',3,'DisplayName',['$$k_{\mathrm{g}} = \ $$', num2str(k(i))]);hold on ;
% h1.Color = red;
...
plot(path_error,x(:,3),'Color',color(i,:),'LineWidth',lw,'DisplayName',['$$k_{\mathrm{g}} = \ $$', num2str(k(i))]);hold on ;grid on;
...
% plot(t,x(:,4),'--m','LineWidth',lw);hold on; grid on;
plot(t,x(:,4),'Color',color(i,:),'LineWidth',lw,'DisplayName',['$$k_{\mathrm{g}} = \ $$', num2str(k(i))]);hold on; grid on;
ax4 = gca;
ax4.FontSize = ax_fnt;
box on                        % Switch on the box around the axis
ax4.XColor = 'black';         % Box horizontal lines' color
...
% fig = gcf;
% fig.WindowState = 'maximized';
plot(t,wrapToPi(x(:,3)),'Color',color(i,:),'LineWidth',lw,'DisplayName',['$$k_{\mathrm{g}} = \ $$', num2str(k(i))]);hold on ;grid on;
% plot(t,x(:,8)./Vgd,'b','linewidth',3);hold on ;grid on;
...
% fig = gcf;
% fig.WindowState = 'maximized';
h6 = plot(t,x(:,4)./Vgd,'Color',color(i,:),'linewidth',3,'DisplayName',['$$k_{\mathrm{g}} = \ $$', num2str(k(i))]);hold on ;grid on;
% plot(t,x(:,8)./Vgd,'b','linewidth',3);hold on ;grid on;
...
% fig = gcf;
% fig.WindowState = 'maximized';
h7 = plot(path_error,x(:,4)./Vgd,'Color',color(i,:),'LineWidth',lw,'DisplayName',['$$k_{\mathrm{g}} = \ $$', num2str(k(i))]);hold on ;grid on;
% plot(t,x(:,8)./Vgd,'b','linewidth',3);hold on ;grid on;
...
function out = fun_sinusoidal(t,x,k2)
global Vgd kv wx wy k_kai k_kaidot A omega
