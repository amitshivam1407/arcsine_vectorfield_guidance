%-------------------------------------------------------------------------%
%---------------------     15th December 2023      -----------------------%
%----- Parabolic path following using Adaptive Sliding Mode Control ------%
%-------------------------------------------------------------------------%
...
global  h k  Va k_s W psi_w A_w A_phiw epsilon nabla zeta_0 zeta_1 zeta_2 c0 c1 c2 c3 ...
    wxs wys tez sz uz kai_ddot mu variance
...
range      = -100:5:100;
[X,Y]      = meshgrid(range);
kaid       = vf_parabola(X,Y,a_arr(1,1));
Xdot       = Va*cos(kaid);
Ydot       = Va*sin(kaid);
...
x_initial = [x_0;y_0;e10;e20;e10_hat;e20_hat;err_10;err_20;x60;x70];

options = odeset('RelTol',1e-8,'AbsTol',1e-8);
[t,x] = ode45(@(t,x)fun_parabola(t,x,u,v,Time,a) ,tspan, x_initial,options);
x_ini = x(1,1) ;
y_ini = x(1,2) ;
x_end = x(end,1) ;
y_end = x(end,2) ;

[kai_p,kai_o,kai_des, kaidot_des] = fun_propkaid(x(:,1),x(:,2),a);
kai = x(:,3) + kai_des ;
% Vg = Wx.*cos(kai) + Wy.*sin(kai) + sqrt(Va^2 - (Wx.*sin(kai) - Wy.*cos(kai))) ;
V_g = get_Vg(t,kai,u,v,Time);
kappa_des = kaidot_des./V_g ;
e_path = -((x(:,2) - k) - a_arr.*(x(:,1) - h).^2);

for j = 1:length(t)
    if abs(e_path(j,1))  <= 0.1
%         c(i) = abs(e_path(i,1)) ;
        c_arr = [c_arr abs(e_path(j,1))];
    else
        c_tran =[c_tran abs(e_path(j,1))];
%         c_arr_col(j,:) = c_arr ;
    end
...
%% Colors
    ...
    color = [red;blue;maroon;green];



figure(1)
% quiver(X,Y,Xdot,Ydot,'color',[0.75  0.75   0.75],'linewidth',1);hold on;
h11 = plot(xd,yd,'-','LineWidth',lw,'Color',color(i,:),'DisplayName', ['$$a = $$' , num2str(a_arr(:,i))]);hold on;grid on;
h12 = plot(x(:,1),x(:,2),'--','LineWidth',lw,'Color',color(i,:),'DisplayName', ['$$a = $$' , num2str(a_arr(:,i))]);hold on;grid on;
...
figure(2)
plot(t,e_path,'-','LineWidth',lw,'Color',color(i,:),'DisplayName', ['$$a = $$' , num2str(a_arr(:,i))]);hold on;grid on;
ax2 = gca;
ax2.FontSize = ax_fnt;
% Outer box setup
box on                        % Switch on the box around the axis
...
figure(3)
plot(t,wrapToPi(kai_des)*(180/pi),'-','LineWidth',lw,'Color',color(i,:),'DisplayName', ['$$a = $$' , num2str(a_arr(:,i))]);hold on;grid on;
plot(t,wrapToPi(kai)*(180/pi),'--','LineWidth',lw,'Color',color(i,:),'DisplayName', ['$$a = $$' , num2str(a_arr(:,i))]);hold on;grid on;
plot(t,wrapToPi(kai_p)*(180/pi),'-.','LineWidth',lw,'Color',color(i,:),'DisplayName', ['$$a = $$' , num2str(a_arr(:,i))]);hold on;grid on;
plot(t,wrapToPi(kai_o)*(180/pi),'-s','LineWidth',lw,'Color',color(i,:),'DisplayName', ['$$a = $$' , num2str(a_arr(:,i))]);hold on;grid on;
ax3 = gca;
ax3.FontSize = ax_fnt;
box on                        % Switch on the box around the axis
...
% 
% figure(6)
% % plot(tez,sz,'linewidth',3,'DisplayName',['Case ', num2str(j)]);grid on;hold on;
% plot(tez,sz,'linewidth',3,'-','LineWidth',lw,'Color',color(i,:),'DisplayName', ['$$a = $$' , num2str(a_arr(:,i))]);hold on;grid on;
% ax6 = gca;
% ax6.FontSize = ax_fnt;
% % Outer box setup
% box on                        % Switch on the box around the axis
...
function out = fun_parabola(t,x,u,v,Time,a)
global Va h k W psi_w A_w A_phiw c0 c2 c1 epsilon tez uz zeta_1 zeta_2 sz k_s wxs wys nabla kai_ddot
...
del_bar = c0*abs(x(5))  + c2*abs(x(6)) ;

rho_est = x(9)*abs(x(5)) + x(10)*abs(x(6)) ;

s = c1*x(5) + x(6);

z_sat = s./(abs(s) + epsilon);

u_z = -c1*x(6) - z2 -nabla*s - rho_est*z_sat ;
...
out(9,1) = abs(x(5))*abs(s)- zeta_1*x(9);
out(10,1) = abs(x(6))*abs(s) -  zeta_2*x(10);
...
function [kai0,kaidot0] = get_initial(xa_0,ya_0,a)
global Va  k_s h k 
...
function [kaid] = vf_parabola(X,Y,a)
global Va  k_s h k
...
% epsilon =  Y - (X.^2)./(4*a);
epsilon =  -((Y-k) - a*(X-h).^2) ;
kai_o = pi/2 - asin(1./(1 + k_s*(epsilon).^2));
for i = 1:length(X)
    for j = 1:length(X)
        if epsilon(i,j)< 0
...
function [kai_p,kai_o,kai_des, kaidot_des] = fun_propkaid(x1,x2,a)
global  Va k_s h k
