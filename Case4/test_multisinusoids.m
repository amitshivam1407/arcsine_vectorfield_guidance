...
global A_amp  Va k_s W psi_w A_w A_phiw epsilon nabla zeta_0 zeta_1 zeta_2 c0 c1 c2 c3 ...
...
omega_arr = [0 0.05 0.1 0.5];
%-------------------------------------------------------------------------%
 
% %----------------  vector field construction  ----------------------------%
% range      = -100:5:100;
% [X,Y]      = meshgrid(range);
% dy_ddx_des = A_amp*omega_arr(1,3)*cos(omega_arr(1,3).*X);
% kai_p_des  = atan(dy_ddx_des);
% e    =  A_amp*sin(omega_arr(1,3).*X)-Y;
% [kaid]     = vf_sinusoidal(X,Y,omega_arr(1,3));
% Xdot       = Va*cos(kaid);
% Ydot       = Va*sin(kaid);
xd = -100:.005:100;
...
[t,x] = ode45(@(t,x)fun_sinusoidal(t,x,u,v,Time,omega) ,tspan, x_initial,options);

e_path = A_amp*sin(omega.*x(:,1)) - x(:,2);
...
h11 = plot(xd,yd,'-','LineWidth',lw,'Color',color(i,:),'DisplayName', ['$$\omega = $$' , num2str(omega_arr(:,i))]);hold on;grid on;
h12 = plot(x(:,1),x(:,2),'--','linewidth',3,'Color',color(i,:),'DisplayName',['$$\omega = $$ ', num2str(omega_arr(:,i))]);hold on;
...
figure(2)
plot(t,e_path,'LineWidth',3,'Color',color(i,:),'DisplayName',['$$\omega = $$ ', num2str(omega_arr(:,i))]);hold on;grid on;
ax2 = gca;
ax2.FontSize = ax_fnt;
% Outer box setup
box on                        % Switch on the box around the axis
...
function out = fun_sinusoidal(t,x,u,v,Time,omega)
global Va A_amp W psi_w A_w A_phiw c0 c2 c1 epsilon tez uz zeta_1 zeta_2 sz k_s Va wxs wys nabla kai_ddot
...
function [kai0,kaidot0] = get_initial(xa_0,ya_0,omega)
global Va  k_s A_amp  
...
function [kaid] = vf_sinusoidal(X,Y,omega)
global Va  k_s A_amp  
...
function [kai_p,kai_o,kai_des, kaidot_des] = fun_propkaid(x1,x2,omega)
global  Va k_s A_amp  
