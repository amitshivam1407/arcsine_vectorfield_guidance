...
global  Vgd k_kai k_kaidot figure_type_good k2_vf k_Nelson_vf
Vgd = 10;
k_kai = 100;  
...
k2_vf = 0.005; % Use middle value for vector field visualization

for i = 1:length(k2)

if x0 < 0
   kai0(i) =  asin(1./(1+ k2(i) *(x0).^2)) ;
   kaidot0(i) = -(2*k2(i)*Vgd.*x0)./((1+k2(i)*(x0).^2).^2);
else
   kai0(i) = pi - asin(1./(1 + k2(i) *(x0).^2)) ;  
   kaidot0(i) = - (2*k2(i)*Vgd.*x0)./((1 + k2(i)*(x0).^2).^2);
end
...
%%%%%%%%%%%%%%%%%  ode solver   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[t,x] = ode45(@(t,x)fun_stline_upd(t,x,k2(i)) ,tspan, x_initial,options);
...
f1;
if i == 1
quiver(X,Y,xdot,ydot,0.7,'color',[0.2 0.6 0.8],'linewidth',1.5);hold(ax1,'on');
...
h1(i) = plot(ax1,x(:,1),x(:,2),'linewidth',3,'DisplayName',['$$k_{\mathrm{s}} = \ $$', num2str(k2(i))]);hold(ax1,'on'); 
h12 = plot(ax1,x_ini,y_ini,'-o','LineWidth',2,'MarkerSize',10,...
    'MarkerEdgeColor','k',...
    'MarkerFaceColor',green); hold(ax1,'on');
h12.Annotation.LegendInformation.IconDisplayStyle = 'off';
h13 = plot(ax1,x_end,y_end,'-s','LineWidth',2,'MarkerSize',10,...
    'MarkerEdgeColor','k',...
...
legend(ax1,'Vector field','Desired path','','Initial position','Final position','FontSize',leg_fnt,'Location','southeast');
axis(ax1,'equal')
...
h41 = plot(ax4,x_prop(i),val_prop(i),'-s','LineWidth',2,'MarkerSize',10,...
...
for i = 1:length(x)
    for j = 1:length(x)
        if x(i,j) < 0
            kaid(i,j) = asin(1./(1+k2_vf*(x(i,j)).^2));
            kaid_dot(i,j) = 2*k2_vf*Vgd.*(x(i,j))./((1+k2_vf*(x(i,j)).^2).^2);
        else
            kaid(i,j) = pi - asin(1./(1+k2_vf*(x(i,j)).^2));
            kaid_dot(i,j) = -2*k2_vf*Vgd.*(x(i,j))./((1+k2_vf*(x(i,j)).^2).^2);
        end
    end
end
...
function [kaid,kaid_dot] = vf_nelson(x)
global Vgd k_Nelson_vf
for i = 1:length(x)
    for j = 1:length(x)
        kaid(i,j) = pi/2 + atan(k_Nelson_vf*x(i,j));
        kaid_dot(i,j) = -(k_Nelson_vf^2*Vgd*x(i,j))./(1+k_Nelson_vf^2.*x(i,j).^2).^(3/2);
    end
end
end

function out = fun_stline_upd(t,x,k2)
global Vgd k_kai k_kaidot
